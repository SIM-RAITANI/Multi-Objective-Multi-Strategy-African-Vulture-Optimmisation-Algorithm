...
# ---------------------------------------------------------------------------
# Quick demo / benchmark comparison
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    configs = [
        # (label,              elite_pool,  improved_F)
        ("Plain AVOA",          False,       False),
        ("AVOA + ElitePool",    True,        False),
        ("AVOA + ImprovedF",    False,       True),
        ("AVOA + Both (MIAVOA)",True,        True),
    ]

    col_w = 28
    hdr = f"{'Function':<38}" + "".join(f"{c[0]:>{col_w}}" for c in configs)
    print("=" * (38 + col_w * len(configs)))
    print(hdr)
    print("=" * (38 + col_w * len(configs)))

    for name, func, dim, lo, hi in problems:
        row = f"{name:<38}"
        for label, ep, imf in configs:
            avoa = AVOA(
                obj_func=func,
                dim=dim,
                lb=lo,
            ...
            _, best_fit = avoa.run(seed=42)
            row += f"{best_fit:>{col_w}.4e}"
        print(row)

    print("=" * (38 + col_w * len(configs)))
    print("\nImprovements active by default:")
    print("  Improvement 3 – Elite Candidate Pool  (use_elite_pool=True)")
    print("  Improvement 4 – Improved hunger param (use_improved_F=True)")
    print("Set either flag to False to disable the corresponding improvement.")